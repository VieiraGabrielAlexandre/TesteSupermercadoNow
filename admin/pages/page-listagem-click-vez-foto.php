...
function show_checkbox(){
	if($('#todos').prop('checked')){
		...
		$('.fcheckbox').prop('checked', false);
		$('.btn-acao').hide();
	}
}

function check_individual(e){
	if($(e).prop('checked')){
		$('.btn-acao').show();
	} else {
		$('.btn-acao').hide();
	...
	<?php $tb_footer = array();?>
<form method="POST" name="form-acao">
	<div class="btn-acao" style="display: none">
		<button type="submit" value="aprovar" onclick="$('#action').val('aprovar')" class="btn btn-primary">APROVAR <i class="icon-arrow-right14 icon-checkmark3"></i></button>
		<button type="submit" value="reprovar" onclick="$('#action').val('reprovar')" style="float: right" class="btn btn-warning">REPROVAR <i class="icon-cross2 position-right"></i></button> <br/><br/>
	</div>
