...
$(function() {    
    ...
    function trafficSources(element, height) {
        ...
        d3.csv("assets/demo_data/dashboard/traffic_sources.csv", function (error, data) {
            ...
            // Hide first tick
            if(!svg.selectAll('.d3-axis-left .tick text').length) return false;
    ...
    function appSalesLines(element, height) {
        ...
        function redraw() { 
            ...
            if(!d3.keys(data[0])) return false;
    ...
    function salesHeatmap() {
        ...
        d3.csv("assets/demo_data/dashboard/app_sales_heatmap.csv", function(error, data) {
            ...
            if(d3Container.node())
                width = d3Container.node().getBoundingClientRect().width - margin.left - margin.right;
            var gridSize = width / new Date(data[data.length - 1].date).getHours(); // dynamically set grid size
            var rowGap = 40; // vertical gap between rows
            var height = (rowGap + gridSize) * (d3.max(nest, function(d,i) {return i+1})) - margin.top;
            var buckets = 5; // number of colors in range
            var colors = ["#DCEDC8","#C5E1A5","#9CCC65","#7CB342","#558B2F"];
